  ...
  return (
    <button
      ...
      className={`flex h-full flex-col rounded-3xl border p-6 text-left transition hover:-translate-y-0.5 hover:shadow-lg ${
        isSelected
          ? 'border-indigo-500 bg-indigo-500/10 shadow-lg ring-2 ring-indigo-400/50'
          : 'border-slate-200 bg-white hover:border-indigo-300 dark:border-slate-800 dark:bg-slate-950'
      }`}
    >
      ...
      <p className="mt-4 text-sm font-medium text-indigo-600 dark:text-indigo-300">{formatHint}</p>
    </button>
  )
}
